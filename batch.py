...
def process_tweet(twitter, tweet):
    ...
    if has_pattern(text):
        ...
        twitter.update_status(status=convert(text),
                              media_ids=[media_id])
...
def process_user(user):
    ...
    tweets = twitter.search(q=f"from:{twitter_user['screen_name']} #NintendoSwitch", since_id=user.last_tweet_id) \
        .get('statuses', [])
